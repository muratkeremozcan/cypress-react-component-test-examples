// https://github.com/muratkeremozcan/epic-react/blob/main/03.advanced-react-hooks/src/exercise/04.js
  ...
  return (
    <div ref={containerRef} role="log">
      {messages.map((message, index, array) => (
        <div data-cy={`quote-${message.id}`} key={message.id}>
  ...
  return (
    <div className="messaging-app">
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <button data-cy="add-message" onClick={addMessage}>
          add message
        </button>
        <button data-cy="remove-message" onClick={removeMessage}>
          remove message
        </button>
      </div>
      <hr />
